	...
	<div class="mx-auto flex w-full flex-col justify-center gap-6 sm:w-[350px]">
		<div class="flex flex-col gap-2 text-center">
			<div class="mx-auto flex size-10 items-center justify-center rounded-full border-2 border-black bg-white dark:border-white dark:bg-black">
				<ChevronLeftIcon class="size-5 text-black dark:text-white" />
			</div>
			<h1 class="text-2xl font-semibold tracking-tight text-black dark:text-white">Welcome back</h1>
			<p class="text-sm text-black/60 dark:text-white/60">Login to your account</p>
		</div>
		<!-- Form -->
		<form method="POST" use:enhanceFunction>
			...
			{:else}
				...
				<Button size="sm" class="w-full rounded-full">Sign In with Email</Button>
			{/if}
		</form>
		<!-- Separator -->
		<div class="relative">
		...
		<p class="px-8 text-center text-sm text-black/60 dark:text-white/60">
			<a href="/signup" class="underline underline-offset-4 hover:text-black dark:hover:text-white">
